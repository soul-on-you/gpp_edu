...
StatusCode LoadMatr(Xtype **&matr, int &matr_str, int &matr_column, const string &FileAdress,
                    function<StatusCode(Xtype const *const *, int, int)> fAdditionalCheck = nullptr, ErrInfo *err = nullptr)
{
...
ostream &OutputMatr(ostream &ostr, Xtype const *const *matr, int matr_str, int matr_column,
                    streamsize width = 0, streamsize precision = 6, ios::fmtflags flags = (ios::fmtflags)0)
{
    if (flags)
    {
        ios::fmtflags resetflags = (ios::fmtflags)0;
