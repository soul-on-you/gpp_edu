...
StatusCode CreateMatr(__matrix_elem_t **&matr, int matr_str, int matr_column, int *MemAllocStepID)
{
...
#else
StatusCode CreateMatrix(__matrix_elem_t **&matr, int matr_str, int matr_column, int *MemAllocStepID)
{
    int i = PREPARETORY_STEP;
    matr = nullptr;
    try
    {
                      ...
                      std::function<StatusCode(__matrix_elem_t const *const *, int, int)> fAdditionalCheck, ErrInfo *err)
{
    ...
    if (file.is_open())
    {
...
#endif
        {
            ...
            {
                if (!(status_code = CreateMatrix(matrix, matrix_str, matrix_column, &err->MemAllocStepID)))
                {
                    file.clear();
                    file.seekg(0);
...
__matrix_t CopyMatrix(__matrix_elem_t const *const *matr, int matr_str, int matr_column, StatusCode &status_code, ErrInfo *err)
{
    ...
    if (!(status_code = CreateMatrix(matrix_copy, matr_str, matr_column, &err->MemAllocStepID)))
        for (int i = 0; i < matr_str; i++)
            for (int j = 0; j < matr_column; j++)
