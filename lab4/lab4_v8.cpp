...
void FReadMatr(ifstream &file, Xtype **matr, const int &matr_str, const int &matr_column)
{
    file.clear();
    file.seekg(0);
...
Xtype **CopyMatr(Xtype const *const *matr1, int matr_str, int matr_column, char &status_code, ErrInfo *err = nullptr)
{
    Xtype **matr2;
    if (!(status_code = CreateMatr(matr2, matr_str, matr_column, err)))
        for (int i = 0; i < matr_str; i++)
            for (int j = 0; j < matr_column; j++)
                *(*(matr2 + i) + j) = *(*(matr1 + i) + j);
    return matr2;
...
char WriteMatr(Xtype const *const *matr, string &FileAdress, int matr_str, int matr_column, ErrInfo *err = nullptr)
{
...
int main()
{
    ...
    while (status_code == Good)
    {
        ...
        {
            status_code = Exit; ///или break; без {status_code = Exit; \n continue;}
            continue;
        }
        ...
        else if (status_code == Good)
        {
            ...
            else
            {
                ...
                while (true)
                {
                    if (GetFileName(FileAdress))
                    {
                        if ((status_code = WriteMatr(copymatr, FileAdress, matr_str, matr_column, &err)) != Good) //функция вывода в файл
                        {
                            status_code = ErrCheck(FileAdress, status_code, &err); //функция обработки ошибок
                            continue;
                        }
